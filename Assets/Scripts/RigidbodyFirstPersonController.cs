...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class RigidbodyFirstPersonController : MonoBehaviour
    {
        ...
        public float jumpingPlatformForce;
        ...
        private void Update()
        {
            ...
            if (m_IsGrounded ) // && !enemyFollow.gameover
            {
                ...
                {
                    if (detectGround.onJumpingPlatform)
                    {
                        PowerJump();
                    }
                    else
        ...
        private void FixedUpdate()
        {
            // if(!enemyFollow.gameover){
                ...
                if ((Mathf.Abs(input.x) > float.Epsilon || Mathf.Abs(input.y) > float.Epsilon) && m_IsGrounded && !Wallrunning)
                {
                    if (crouching)
                    {
                        m_RigidBody.AddRelativeForce(0, 0, 0);
                    }
                    else
                    {
        ...
        {
            m_RigidBody.velocity = new Vector3(m_RigidBody.velocity.x, 0f, m_RigidBody.velocity.z);
            m_RigidBody.AddForce(new Vector3(0f, movementSettings.JumpForce * jumpingPlatformForce, 0f), ForceMode.Impulse);
        }
